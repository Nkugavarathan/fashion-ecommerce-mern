...
export default function SearchModal() {
  ...
  const fetchResults = async (q) => {
    ...
    try {
      ...
      // Filter manually for gender and color if backend search is limited
      const lowerQuery = q.toLowerCase()
      data = data.filter((p) => {
        const title = p.title?.toLowerCase() || p.name?.toLowerCase() || ""
        const categories = Array.isArray(p.categories)
          ? p.categories.map((c) => c.toLowerCase())
          : []
        const color = Array.isArray(p.color)
    ...
    <div ref={ref} className="relative w-full max-w-xl">
      <div className="flex items-center border border-gray-300 rounded-full px-4 py-2 bg-white shadow-sm">
        <input
          ...
          onChange={(e) => setQuery(e.target.value)}
          onFocus={() => setOpen(true)}
          placeholder="Search by title, gender, subcategory, or color..."
          className="outline-none w-full text-sm md:text-base placeholder-gray-400"
        />
        <svg
          className="w-5 h-5 text-gray-400"
          ...
          <div className="max-h-80 overflow-auto">
            ...
            {!loading &&
              results.map((p) => {
                ...
                const colors = Array.isArray(p.color) ? p.color.join(", ") : "—"
